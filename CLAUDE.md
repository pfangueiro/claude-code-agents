...
**Autonomous cross-system migration (opt-in):** a `git pull` on any machine is the only manual trigger — the next SessionStart healthcheck / watchdog forks `install.sh --update`, which reconciles `~/.claude` AND (only if `~/.claude/.framework-autonomy` sets `LEGACY_PROJECTS_DIR`) tears down old per-project `.claude` copies there. That teardown is framework-scoped, untracked-only (never rewrites a repo's history), snapshot-first, skips committed repos, and idempotent. No marker → no teardown; there is deliberately **no auto-pull**.
