...
A `~/.claude/settings.json` that is missing, **unparseable, or a valid-JSON non-object** (`[]`, `123`, `"x"`, `true` — anything for which `jq -e 'type == "object"'` fails) is recovered rather than left dead: `sync_hooks` backs the bad file up to `settings.json.corrupt-<timestamp>` and reseeds from the template, so hook and statusline wiring are restored (262c002; extended from the parse-only check to the non-object case in 0c823cd). This complements the `.hooks`-wipe recovery above — that path re-populates an empty `{}`; this one handles a file that is not a usable object at all. Both `install.sh` (via `sync_hooks`) and `validate.sh` gate on `jq -e 'type == "object"'` rather than `jq -e .`, because the latter treats `[]`/`123`/`"x"`/`true` as "valid" and would never trigger the reseed. Verified by injecting each shape and confirming the reseed fires while an already-valid object is never spuriously replaced.
